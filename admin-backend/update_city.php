...
if (isset($_POST['submit'])) {
    ...
    $user_exist = "SELECT `name` FROM `city` WHERE name='$name' AND id!='$id'";
    ...
    if ($user_exist_query->num_rows >= 1) {
        ...
        if ($processError == false) {
            ...
            if ($user_id) {
                $update_sql = "UPDATE `city` SET name='$name', updated_at=CURRENT_TIMESTAMP, updated_by='$user_id' WHERE id=$id";
